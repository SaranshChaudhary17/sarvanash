...
export default function AboutClient({ initialAboutData }: { initialAboutData: AboutData | null }) {
  const [aboutData, setAboutData] = useState<AboutData>(initialAboutData || DEFAULT_ABOUT);

  // Sync client-side in case live changes are saved in another tab
  useEffect(() => {
    fetch("/api/admin")
      .then((res) => {
      ...
      .catch((err) => console.error("Failed to sync about data", err));
  }, []);
